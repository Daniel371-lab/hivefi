...
class _GastoScreenState extends State<GastoScreen> {
  ...
  final List<Map<String, dynamic>> _historialReciente = [
    {'descripcion': 'Gasto en COMIDA', 'monto': 30000.0},
    {'descripcion': 'Gasto en COMIDA', 'monto': 10000.0},
  ];
  ...
  double get _disponible {
    final cat = _categorias.firstWhere((c) => c['id'] == _categoriaSeleccionada, orElse: () => _categorias.first);
  ...
  Future<void> _confirmarGasto() async {
    ...
    if (monto > _disponible) { setState(() => _errorMessage = context.tr('notEnoughFunds')); return; }
    setState(() { _isLoading = true; _errorMessage = null; });
    await Future.delayed(const Duration(milliseconds: 600));
    if (!mounted) return;
    setState(() => _isLoading = false);
    _montoController.clear();
    ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(context.tr('expenseRegistered')), backgroundColor: Theme.of(context).colorScheme.primary, behavior: SnackBarBehavior.floating));
  }
          ...
          body: SafeArea(
            child: SingleChildScrollView(
              ...
              child: Column(
                ...
                children: [
                  Text(context.tr('registerExpense'), style: theme.textTheme.headlineLarge?.copyWith(fontWeight: FontWeight.w700, height: 1.1)),
                  const SizedBox(height: 32),
                  Text(context.tr('whereMoneyLeaves'), style: theme.textTheme.bodySmall?.copyWith(color: honey, fontWeight: FontWeight.w700, letterSpacing: 1.2)),
                  ...
                  Text(context.tr('expenseAmount'), style: theme.textTheme.bodySmall?.copyWith(color: honey, fontWeight: FontWeight.w700, letterSpacing: 1.2)),
                  const SizedBox(height: 8),
                  TextField(
                    controller: _montoController,
                    keyboardType: TextInputType.number,
                    textInputAction: TextInputAction.done,
                    onSubmitted: (_) => _confirmarGasto(),
                  ...
                  SizedBox(
                    ...
                    child: ElevatedButton(
                      onPressed: _isLoading ? null : _confirmarGasto,
                      child: _isLoading
                          ? const SizedBox(width: 22, height: 22, child: CircularProgressIndicator(strokeWidth: 2.5, color: Colors.white))
                          : Text(context.tr('confirmExpense'), style: const TextStyle(fontWeight: FontWeight.w700, letterSpacing: 1)),
                    ),
                  ),
                  const SizedBox(height: 32),
      ...
      child: Column(
        ...
        children: [
          ...
          ...historial.map((item) => Padding(
            ...
            child: Text('${item['descripcion']} (-${CurrencyFormatter.format(item['monto'] as double, currency)})', style: theme.textTheme.bodyMedium?.copyWith(fontWeight: FontWeight.w500, color: Colors.red)),
          )),
        ],
      ),
    );
